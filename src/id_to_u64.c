...
id_to_u64 * id_to_u64_create(const char * optional_name){
  ...
  static const char * const column_types[] = {"object_id", "u64"};
  id_to_u64 * instance = calloc(sizeof(id_to_u64), 1);
  instance->column_names = (char **)column_names;
  instance->column_types = (char **)column_types;
  
  icy_table_init((icy_table * )instance, optional_name, 2, (unsigned int[]){sizeof(object_id), sizeof(u64)}, (char *[]){(char *)"key", (char *)"value"});
...
bool id_to_u64_try_get(id_to_u64 * table, object_id * key, u64 * value){
  ...
  unsigned int sizes[] = {sizeof(object_id), sizeof(u64)};
  for(int i = 1; i < 2; i++){
    if(array[i] != NULL)
