// This file is auto generated by icy-table.
#ifndef TABLE_COMPILER_INDEX
#define TABLE_COMPILER_INDEX
#define array_element_size(array) sizeof(array[0])
#define array_count(array) (sizeof(array)/array_element_size(array))
#include "icydb.h"
#include <stdlib.h>
#endif


interned_strings * interned_strings_create(const char * optional_name){
  static const char * const column_names[] = {(char *)"key", (char *)"str"};
  static const char * const column_types[] = {"u32", "string_table_indexes"};
  interned_strings * instance = calloc(sizeof(interned_strings), 1);
  instance->column_names = (char **)column_names;
  instance->column_types = (char **)column_types;
  
  icy_table_init((icy_table * )instance, optional_name, 2, (unsigned int[]){sizeof(u32), sizeof(string_table_indexes)}, (char *[]){(char *)"key", (char *)"str"});
  
  return instance;
}

void interned_strings_insert(interned_strings * table, u32 * key, string_table_indexes * str, size_t count){
  void * array[] = {(void* )key, (void* )str};
  icy_table_inserts((icy_table *) table, array, count);
}

void interned_strings_set(interned_strings * table, u32 key, string_table_indexes str){
  void * array[] = {(void* )&key, (void* )&str};
  icy_table_inserts((icy_table *) table, array, 1);
}

void interned_strings_lookup(interned_strings * table, u32 * keys, size_t * out_indexes, size_t count){
  icy_table_finds((icy_table *) table, keys, out_indexes, count);
}

void interned_strings_remove(interned_strings * table, u32 * keys, size_t key_count){
  size_t indexes[key_count];
  size_t index = 0;
  size_t cnt = 0;
  while(0 < (cnt = icy_table_iter((icy_table *) table, keys, key_count, NULL, indexes, array_count(indexes), &index))){
    icy_table_remove_indexes((icy_table *) table, indexes, cnt);
    index = 0;
  }
}

void interned_strings_clear(interned_strings * table){
  icy_table_clear((icy_table *) table);
}

void interned_strings_unset(interned_strings * table, u32 key){
  interned_strings_remove(table, &key, 1);
}

bool interned_strings_try_get(interned_strings * table, u32 * key, string_table_indexes * str){
  void * array[] = {(void* )key, (void* )str};
  void * column_pointers[] = {(void *)table->key, (void *)table->str};
  size_t __index = 0;
  icy_table_finds((icy_table *) table, array[0], &__index, 1);
  if(__index == 0) return false;
  unsigned int sizes[] = {sizeof(u32), sizeof(string_table_indexes)};
  for(int i = 1; i < 2; i++){
    if(array[i] != NULL)
      memcpy(array[i], column_pointers[i] + __index * sizes[i], sizes[i]); 
  }
  return true;
}

void interned_strings_print(interned_strings * table){
  icy_table_print((icy_table *) table);
}

size_t interned_strings_iter(interned_strings * table, u32 * keys, size_t keycnt, u32 * optional_keys_out, size_t * indexes, size_t cnt, size_t * iterator){
  return icy_table_iter((icy_table *) table, keys, keycnt, optional_keys_out, indexes, cnt, iterator);

}
